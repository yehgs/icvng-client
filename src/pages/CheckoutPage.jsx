...
const CheckoutPage = () => {
  ...
  // Test function you can run in browser console or create as a separate page

  const testPickupMethodDirectly = async () => {
    try {
      console.log('Testing pickup method...');

      // Replace these with actual IDs from your database
      const testData = {
        addressId: '68711f0a601e02865b891178', // Replace with real address ID
        items: [
          {
            productId: '687a2a0fb32f9c55b2443c79', // Replace with real product ID
            quantity: 1,
            category: '67e3e95a7253e0426126be80', // Replace with real category ID (optional)
            weight: 1,
            name: 'Test Product',
          },
        ],
      ...
      const debugResponse = await fetch(
        '/api/shipping/debug-calculate-checkout',
        {
          method: 'POST',
          headers: {
      ...
      const actualResponse = await fetch('/api/shipping/calculate-checkout', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      console.log('Actual Response:', JSON.stringify(actualResult, null, 2));

      return { debug: debugResult, actual: actualResult };
    } catch (error) {
  ...
  // Test specific pickup method
  const testSpecificPickup = async () => {
    try {
      const response = await fetch('/api/shipping/test-specific-pickup');
      const result = await response.json();
      console.log('Specific pickup test:', JSON.stringify(result, null, 2));
      return result;
    } catch (error) {
      console.error('Specific pickup test failed:', error);
      return { error: error.message };
    }
  };

  // Test zone lookup
  const testZone = async (city = 'Lagos', state = 'Lagos') => {
    try {
      const response = await fetch(
        `/api/shipping/test-zone-lookup?city=${city}&state=${state}`
      );
      const result = await response.json();
      console.log('Zone lookup test:', JSON.stringify(result, null, 2));
      return result;
    } catch (error) {
      console.error('Zone lookup test failed:', error);
      return { error: error.message };
    }
  };
  ...
  const runAllTests = async () => {
    console.log('=== Running all shipping tests ===');

    const zoneTest = await testZone();
    const specificTest = await testSpecificPickup();
    const fullTest = await testPickupMethodDirectly();

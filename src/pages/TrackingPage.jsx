...
const TrackingPage = () => {
  ...
  const handleTrackingSearch = async (number = searchInput) => {
    if (!number.trim()) {
      toast.error(t("tracking.enterTrackingNumberError"));
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setError(response.data.message || t("tracking.trackingNotFound"));
        setTracking(null);
      }
    } catch (error) {
      setError(error.response?.data?.message || t("tracking.failedToTrack"));
      setTracking(null);
  ...
  const getStatusDescription = (status) => {
    const descriptions = {
      ...
      ATTEMPTED: t("tracking.statusAttempted"),
      RETURNED: t("tracking.statusReturned"),
      LOST: t("tracking.statusLost"),
      CANCELLED: t("tracking.statusCancelled"),
    };
    return descriptions[status] || t("tracking.statusUnknown");
  };
              ...
              >
                {loading ? (
                  <div className="flex items-center">
                    ...
                    {t("tracking.trackingInProgress")}
                  </div>
                ) : (
                  t("tracking.trackingButton")
                )}
              </button>
            </div>
          </div>
            ...
            <div className="flex items-center">
              ...
              <div>
                <h3 className="text-lg font-medium text-red-800">
                  {t("tracking.trackingNotFoundHeading")}
                </h3>
                <p className="text-red-700 mt-1">{error}</p>
              </div>
            </div>
            <div className="mt-4 text-sm text-red-700">
              <p className="font-medium">{t("tracking.pleaseCheck")}</p>
              <ul className="list-disc list-inside mt-2 space-y-1">
                <li>{t("tracking.checkCorrectNumber")}</li>
                <li>
                  {t("tracking.checkShipped")}
                </li>
                <li>
                  {t("tracking.checkCorrectSource")}
                </li>
              </ul>
            </div>
          </div>
              ...
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900">
                    {t("tracking.trackingLabel")} {tracking.trackingNumber}
                  </h2>
                  <p className="text-gray-600">
                    {t("tracking.carrierLabel")} {tracking.carrier.name}
                  </p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                {tracking.actualDelivery && (
                  <div className="text-center p-4 bg-green-50 rounded-lg">
                    ...
                    <div className="text-sm text-green-600">{t("tracking.deliveredOn")}</div>
                ...
                <div className="text-center p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="text-sm text-gray-600">{t("tracking.packageWeight")}</div>
                  <div className="text-lg font-semibold text-gray-900">
                ...
                <div className="text-center p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="text-sm text-gray-600">{t("tracking.destination")}</div>
                  <div className="text-lg font-semibold text-gray-900">
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                <p className="text-sm text-gray-600">
                  <strong>{t("tracking.supportHoursLabel")}</strong> {t("tracking.supportHours")}
                </p>
                <p className="text-sm text-gray-600 mt-1">
                  {t("tracking.urgentIssueNote")}
                </p>
              </div>
            </div>
          </div>
        ...
        {!tracking && !loading && !error && (
          <div className="bg-white rounded-lg shadow-sm p-6">
            <h3 className="text-lg font-medium text-gray-900 mb-4">
              {t("tracking.noTrackingNumberHeading")}
            </h3>
            <p className="text-gray-600 mb-4">
              {t("tracking.noTrackingNumberMessage")}
            </p>

            <div className="space-y-2">
              <p className="text-sm font-medium text-gray-700">
                {t("tracking.trySampleNumbers")}
              </p>
